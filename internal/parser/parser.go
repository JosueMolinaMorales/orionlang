...
func (p *Parser) parseExpressionList(end token.TokenType) []ast.Expression {
	...
	// If the next token is the end token, return list
	...
	// While the next token is a comma, keep parsing expressions
...
func (p *Parser) parseFunctionParameters() []*ast.Identifier {
	...
	// If the next token is a closing parenthesis, we know
	...
	// While the next token is a comma, keep parsing identifiers
	for p.peekTokenIs(token.COMMA) {
		// Skip the comma
		p.nextToken()
		// Go to identifer
...
func (p *Parser) parseExpression(precendence int) ast.Expression {
	// Get the prefix parse function for the current token type
	...
	// Loop until the next token is a semicolon or
	// the precendence of the next token is greater than
	// the current precendence
