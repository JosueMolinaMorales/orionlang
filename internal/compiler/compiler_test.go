	...
	tests := []compilerTestCase{
		{
			...
			expectedConstants: []interface{}{
				...
				[]code.Instructions{
					...
					code.Make(code.OpReturnValue), // The return value
				},
			},
			expectedInstructions: []code.Instructions{
		...
		},
		{
			...
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 1),  // The compiled function
				code.Make(code.OpSetGlobal, 0), // The global variable
				code.Make(code.OpGetGlobal, 0), // The global variable
				code.Make(code.OpCall, 0),
				code.Make(code.OpPop),
			},
		},
...
func TestCompilerScopes(t *testing.T) {
	...
	if last.Opcode != code.OpAdd {
		t.Errorf("lastInstruction.Opcode wrong. got=%d, want=%d", last, code.OpAdd)
	}

	previous := compiler.scopes[compiler.scopeIndex].previousInstruction
	if previous.Opcode != code.OpMultiply {
		t.Errorf("previousInstruction.Opcode wrong. got=%d, want=%d", previous, code.OpMultiply)
	}
}
...
func TestStringExpressions(t *testing.T) {
	tests := []compilerTestCase{
		{
			input:             `"orion"`,
			expectedConstants: []interface{}{"orion"},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpPop),
			},
		...
		{
			input:             `"orion" + "lang"`,
			expectedConstants: []interface{}{"orion", "lang"},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
				code.Make(code.OpAdd),
...
func TestIntegerArithmetic(t *testing.T) {
	tests := []compilerTestCase{
		...
		{
			input:             "5 - 4",
			expectedConstants: []interface{}{5, 4},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
				code.Make(code.OpSubtract),
		...
		{
			input:             "5 / 5",
			expectedConstants: []interface{}{5, 5},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
...
func testInstructions(expected []code.Instructions, actual code.Instructions) error {
	...
	for i, ins := range concatted {
		if actual[i] != ins {
			return fmt.Errorf("wrong instruct at %d.\nwant=%q\ngot =%q", i, concatted, actual)
		}
	}
